source ${HOME}/.s2i/bin/launch/tx-datasource.sh
...
function inject_datasource() {
  ...
  case "$db" in
    "MYSQL")
      local ${prefix}_XA_CONNECTION_PROPERTY_Port
      local ${prefix}_XA_CONNECTION_PROPERTY_ServerName
      local ${prefix}_XA_CONNECTION_PROPERTY_DatabaseName
      ...
      if [ -z "$(eval echo \$${prefix}_XA_CONNECTION_PROPERTY_URL)" ]; then
        ...
        if [ "x${!portVar}" == "x" ]; then
          eval ${prefix}_XA_CONNECTION_PROPERTY_Port=${port}
        fi
        if [ "x${!serverNameVar}" == "x" ]; then
          eval ${prefix}_XA_CONNECTION_PROPERTY_ServerName=${host}
        fi
        if [ "x${!databaseNameVar}" == "x" ]; then
          eval ${prefix}_XA_CONNECTION_PROPERTY_DatabaseName=${database}
        fi
      fi
      ...
      ;;
    "POSTGRESQL")
      local ${prefix}_XA_CONNECTION_PROPERTY_PortNumber
      local ${prefix}_XA_CONNECTION_PROPERTY_ServerName
      local ${prefix}_XA_CONNECTION_PROPERTY_DatabaseName
