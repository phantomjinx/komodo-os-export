...
function processEnvFiles() {
  debug "Processing Environment Files ..."
    ...
    (
      ...
      for prop_file_arg in $(echo $ENV_FILES | sed "s/,/ /g"); do
        for prop_file in $(find $prop_file_arg -maxdepth 0 2>/dev/null); do
          (
            if [ -f $prop_file ]; then
              debug "Processing Environment file $prop_file"
              source $prop_file
              executeModules preConfigureEnv
              executeModules configureEnv
              executeModules postConfigureEnv
            else
              echo "Could not process environment for $prop_file.  File does not
 exist."
            fi
          )
        done
...
debug "Starting executeModules.preConfigure"
executeModules preConfigure
debug "Starting executeModules.configure"
executeModules configure
debug "Starting processEnvFiles"
processEnvFiles
debug "Starting executeModules.postConfigure"
